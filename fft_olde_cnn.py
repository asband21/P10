...
def lode_data(split: float = 0.1, min_fri: float = 400, spl_amt: int = 3, seed: int = 5212, batch_size: int = 64):
    with open("archive_1_data.csv", "r") as f:
        rows = [line.strip().split("\t") for line in f]

    bash = []
    for bash_num in range(0, int(len(rows)), batch_size):
        images = []
        labels  = []
        distens = [] 
        batch_rows = rows[bash_num:bash_num + batch_size]
        for i in batch_rows:
            ...
            x_fft_0 = jnp.abs(jnp.fft.rfft(x_spl_0, axis=-1)) #.flatten()
            x_fft_1 = jnp.abs(jnp.fft.rfft(x_spl_1, axis=-1)) #.flatten()

            #x_fft_0 = jnp.fft.rfft(x_spl_0, axis=-1).flatten()
            #x_fft_1 = jnp.fft.rfft(x_spl_1, axis=-1).flatten()
        ...
        bash.append([images_train, label_train, images_test, label_test, mapped_labels, n_chunks])
    return bash
...
for epoch in range(256):
    n = 0
    for (images_train, label_train, images_test, label_test, mapped_labels, n_chunks) in bash :
        ...
        n = n + 1 
        if epoch % 4 == 0 and n == 1:  # Print metrics.
