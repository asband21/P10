...
def traning(i, carry):
    a, b, fun_input, true, error_over_time = carry
    
    a = a - l*grad_network_a_error(a, b, fun_input[i], true[i])
    b = b - l*grad_network_b_error(a, b, fun_input[i], true[i])
    
    #jax.debug.print("a:{} b:{} a_m:{} b_m:{}", a, b, a_m, b_m)
    error_over_time = error_over_time.at[i].set(net(a, b, fun_input[i], true[i]))
    return (a, b, fun_input, true, error_over_time)
...
lkl = (a, b, fun_input, data_set, error_over_time)
val = jax.lax.fori_loop(0, num, traning, lkl) 
a, b, sam, true, error_over_time = val
