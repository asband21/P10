...
def lode_data(split: float = 0.1):
    ...
    for i in rows:
        ...
        x = x.astype("float32")
        x = x / (32768.0)
        #x_spl = jnp.split(x, len(x)/(1024/8))
        x_spl = jnp.split(x, len(x)/(128))
        x_fft = []
        for i_x in x_spl:
    ...
    images_train, label_train, images_test, label_test = jax_train_test_split(images, mapped_labels, test_fraction=split, seed=232)
...
class SimpleNN(nnx.Module):
    def __init__(self, n_features: int = 57720, n_hidden: int = 255, n_targets: int = 26, *, rngs: nnx.Rngs):
