...
class CNN1(nnx.Module):
    def __init__(self, n_targets: int = 26, sh: tuple[int, int] = (10, 10), *, rngs: nnx.Rngs):
        self.conv0 = nnx.Conv(1, 1, kernel_size=(3, 3), padding="VALID", rngs=rngs)
        self.conv1 = nnx.Conv(1, 1, kernel_size=(3, 3), padding="VALID", rngs=rngs)
        self.conv2 = nnx.Conv(1, 1, kernel_size=(3, 3), padding="VALID", rngs=rngs)

        self.output = nnx.Linear((sh[0]-6)*(sh[1]-6), n_targets, rngs=rngs)
...
#model = CNN1(sh=shape, n_targets=26, rngs=nnx.Rngs(0))
