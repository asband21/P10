...
def lode_data(split: float = 0.1, min_fri: float = 20000, seed: int = 5212, batch_size: int = 64, d: int = 1):
    ## make index to data
    with open("./data_set_rumba_3/data_set_clean.csv", "r") as f:
        rows = [line.strip().split("\t") for line in f]
    for lolo in rows:
        lolo[4] = "data_set_rumba_3/" + lolo[4]
        lolo[5] = "data_set_rumba_3/" + lolo[5]
    ...
    for bash_num in range(0, int(len(rows_shuffled)/d), batch_size):
        ...
        labels_bash = np.asarray(labels_bash, dtype=np.float32)
...
for epoch in range(epoch):
    ...
    for im_tr in range(len(images_train)):
        ...
        #print(labels_test)
        #print(idx)
        lab_tra_jnp = jnp.array(labels_train[idx])
        #print(lab_tra_jnp)
        train_step(model, optimizer, noisy_images_train, lab_tra_jnp)
